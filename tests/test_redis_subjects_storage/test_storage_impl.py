...
import json
import pytest
from redis_subjects_storage.storage_impl import SubjectsRedisStorage, create_redis_storage, create_redis_client
...
class TestSubjectsRedisStorage:
    ...
    async def test_storage_initialization(self, redis_storage, subject_name):
    ...
    async def test_is_concurrency_safe(self, redis_storage):
        """Redis storage should be concurrency safe."""
        assert redis_storage.is_concurrency_safe() is True

    async def test_is_persistent(self, redis_storage):
        """Redis storage should be persistent."""
        assert redis_storage.is_persistent() is True

    async def test_load_empty_subject(self, redis_storage):
        """Loading non-existent subject should return empty dicts."""
        default_props, ext_props = await redis_storage.load()

        assert default_props == {}
        assert ext_props == {}

    async def test_store_inserts(self, redis_storage):
        ...
        # Store properties
        await redis_storage.store(inserts=[prop1, prop2, prop3])
    ...
    async def test_store_mixed_operations(self, redis_storage):
    ...
    async def test_set_simple_value(self, redis_storage):
        """Set should work with simple non-callable values."""
        prop = Property("name", "Alice", PropertyType.DEFAULT)

        new_value, old_value = await redis_storage.set(prop, old_value_default=None)
    ...
    async def test_set_update_existing(self, redis_storage):
        ...
        # Update via set
        prop2 = Property("count", 20, PropertyType.DEFAULT)
        new_value, old_value = await redis_storage.set(prop2)

        assert new_value == 20
        assert old_value == 10

    async def test_set_callable_value(self, redis_storage):
    ...
    async def test_set_callable_with_default(self, redis_storage):
        ...
        new_value, old_value = await redis_storage.set(prop, old_value_default=0)

        assert old_value == 0
        assert new_value == 1

    async def test_get_existing_property(self, redis_storage):
        ...
        value = await redis_storage.get(prop2)

        assert value == "Bob"

    async def test_get_non_existent_property(self, redis_storage):
        """Get should raise AttributeError for non-existent property."""
        prop = Property("nonexistent", None, PropertyType.DEFAULT)

        with pytest.raises(AttributeError) as exc_info:
            await redis_storage.get(prop)

        assert "nonexistent" in str(exc_info.value)

    async def test_get_extended_property(self, redis_storage):
    ...
    async def test_delete_existing_property(self, redis_storage):
        ...
        await redis_storage.delete(prop2)

        # Verify deleted
        default_props, _ = await redis_storage.load()
        assert "temp" not in default_props

    async def test_delete_non_existent_property(self, redis_storage):
        """Delete should not raise error for non-existent property."""
        # Delete non-existent property (should not raise)
        prop = Property("nonexistent", None, PropertyType.DEFAULT)
        await redis_storage.delete(prop)  # Should not raise

    async def test_get_ext_props(self, redis_storage):
        ...
        await redis_storage.store(inserts=[default_prop, ext_prop1, ext_prop2])

        # Get extended properties
        ext_props = await redis_storage.get_ext_props()
    ...
    async def test_get_ext_props_empty(self, redis_storage):
    ...
    async def test_flush_deletes_all_properties(self, redis_storage):
    ...
    async def test_key_prefix_isolation(self, redis_storage_factory, redis_client):
...
class TestCreateRedisStorage:
    ...
    async def test_factory_creates_storage_instance(self, redis_client):
